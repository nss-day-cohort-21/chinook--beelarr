...
select Track.Name, count(InvoiceLine.InvoiceId) as 'count' from InvoiceLine join Track, Invoice where InvoiceLine.InvoiceId = Invoice.InvoiceId and Invoice.InvoiceDate like '2013%' and InvoiceLine.TrackId = Track.TrackId  group by Track.name order by count desc limit 9
select Track.Name, count(InvoiceLine.InvoiceId) as 'count' from InvoiceLine join Track, Invoice where InvoiceLine.InvoiceId = Invoice.InvoiceId  and InvoiceLine.TrackId = Track.TrackId  group by Track.name order by count desc limit 5
select Track.Name, count(InvoiceLine.InvoiceId) as 'count' from InvoiceLine join Track, Invoice, Album, Artist where InvoiceLine.InvoiceId = Invoice.InvoiceId  and InvoiceLine.TrackId = Track.TrackId and Track.AlbumId = Album.AlbumId and Album.ArtistId = Artist.ArtistId  group by Artist.Name order by count desc limit 3
select MediaType.Name, count(MediaType.Name) as 'count' from MediaType join Track, InvoiceLine where InvoiceLine.TrackId = Track.TrackId and Track.MediaTypeId = MediaType.MediaTypeId  group by MediaType.Name order by count desc limit 1
